...
class _Gauge(QWidget):
    ...
    def mouseMoveEvent(self, event):
        ...
        QToolTip.showText(event.globalPosition().toPoint(),
                          f"{cat}\n{size / 1024:,.1f} Kio".replace(",", " ") +
                          f"  ·  {pct:.1f} % de la ROM", self)
...
class RomBudgetBar(QWidget):
    ...
    def _refresh(self):
        ...
        self._lbl_rom.setToolTip(
            f"Cartouche visée : {self._cartridge_mib} MiB — cliquer pour changer")
        ...
        pct = 100 * fill_ratio
        self._value.setText(f"{self._kio(report.rom_bytes)} / {self._cartridge_mib} Mio  ({pct:.1f}%)")
