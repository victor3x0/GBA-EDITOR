...
from PyQt6.QtGui import QFont, QCursor, QPixmap, QPainter, QColor, QPen
...
_TOOLTIPS = {
    ...
    "actor.y":           ("self.y",
                          ...
                          "Écriture → self:set_pos(x, y)  ou  self:move(dx, dy)"),
    "actor.dir_x":       ("self.dir_x  /  self:set_direction(dx, dy)",
                          "Direction X discrète : -1 (gauche) | 0 (neutre) | 1 (droite).\n"
                          "Lecture : local dx = self.dir_x\n"
                          "Écriture : self:set_direction(dx, dy)"),
    "actor.dir_y":       ("self.dir_y  /  self:set_direction(dx, dy)",
                          "Direction Y discrète : -1 (haut) | 0 (neutre) | 1 (bas).\n"
                          "Lecture : local dy = self.dir_y\n"
                          "Écriture : self:set_direction(dx, dy)"),
...
class SceneInspector(QWidget):
    ...
    def _on_layer_image(self, layer, path_str: str):
        ...
        if path_str:
            ...
            src = _P(path_str)
            dst = self._project.import_asset(src, "backgrounds")
            layer.image = dst.name
        else:
            layer.image = ""
        self._project.save_background(ba)
        self.changed.emit()
    ...
    def _on_layer_remove(self, layer):
        ...
        ba = self._project.get_background(self._scene.background_asset)
        if not ba: return
        if layer in ba.layers:
            ba.layers.remove(layer)
            self._project.save_background(ba)
        self._rebuild_layer_rows()
        self.changed.emit()
    ...
    def _add_bg_layer(self):
        ...
        ba.layers.append(BackgroundLayer(image="", bg_slot=next_slot, scroll_speed=1.0))
        self._project.save_background(ba)
        self._rebuild_layer_rows()
        self.changed.emit()
    ...
    def _on_bound_toggled(self, idx: int):
        ...
        for row in self._bg_layer_rows:
            row.set_bound(row.slot_index == idx)
        self.changed.emit()
...
class ScriptUsesInspector(QWidget):
    ...
    def load(self, script_path: str, project):
        ...
        def _add_group_row(icon: str, label: str, color: str, count: int | None = None):
            row = QFrame()
        ...
        def _add_leaf_row(label: str, on_click):
            leaf = QFrame()
        ...
        for scene in project.scenes:
            ...
            if not linked:
                continue
            found_any = True
            _add_group_row("✦", scene.name, C.ACCENT_ORG, count=len(linked))
            for actor in linked:
                _add_leaf_row(actor.name, lambda a=actor: get_bus().select(a))
        ...
        linked_prefabs = [
            ...
            and _Path(pf.get_component("script").script or "").name == name
        ]
        if linked_prefabs:
            found_any = True
            _add_group_row("◆", "PREFABS", C.ACCENT_BLU)
            for pf in linked_prefabs:
                _add_leaf_row(pf.name, lambda p=pf: get_bus().select(p))

        # ── Scripts de scène (Scene.script — distinct des scripts d'actor) ──
        linked_scenes = [s for s in project.scenes if _Path(s.script or "").name == name]
        if linked_scenes:
            found_any = True
            _add_group_row("▤", "SCRIPTS DE SCÈNE", C.ACCENT_ORG)
            for scene in linked_scenes:
                _add_leaf_row(scene.name, lambda s=scene: get_bus().select(s))
...
class VariableUsesInspector(QWidget):
    """
    ...
    def __init__(self, parent=None):
        ...
        # ── Header vert avec nom de la variable/constante ─────────
        self._top = QFrame()
        self._top.setFixedHeight(40)
        self._top.setStyleSheet(f"background:{C.ACCENT_GRN}25; border-bottom:1px solid {C.ACCENT_GRN}40;")
        tl = QHBoxLayout(self._top)
        tl.setContentsMargins(12, 0, 8, 0)
        self._name_lbl = QLabel("")
        self._name_lbl.setFont(QFont(T.MONO, T.LG, QFont.Weight.Bold))
        ...
        btn_close.setFixedSize(24, 24)
        btn_close.setStyleSheet(
            f"QPushButton{{color:{C.ACCENT_GRN};background:transparent;border:none;"
            "font-family:monospace;font-size:10px;}"
            f"QPushButton:hover{{color:{C.TEXT_HI};}}"
        )
        btn_close.setToolTip("Fermer cette vue")
        ...
        sl = QHBoxLayout(sec_bar)
        sl.setContentsMargins(10, 0, 8, 0)
        self._sec_lbl = QLabel("GLOBAL USES")
        ...
        # ── Liste des scripts référençant la variable ─────────────
        scroll = QScrollArea()
        scroll.setWidgetResizable(True)
        scroll.setStyleSheet(f"background:{C.BG_PANEL}; border:none;")
        scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
    ...
    def load(self, kind: str, name: str, project):
        ...
        if not found_any:
            empty = QLabel(f"  Aucun script n'utilise « {name} ».")
            empty.setFont(QFont(T.MONO, T.MD))
            empty.setStyleSheet("color:#444; padding:12px;")
            self._list_layout.addWidget(empty)
...
class DynamicInspector(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Bandeau contextuel coloré — composant partagé (voir ui/widgets.py),
