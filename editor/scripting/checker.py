...
class Checker:
    ...
    def check(self, script: LuaScript) -> list[CheckError]:
    ...
    def _check_function(self, fn: LuaFunction):
        if fn.name not in KNOWN_EVENTS:
    ...
    def _call_key(self, func_expr) -> Optional[str]:
        ...
        if isinstance(func_expr, ExprName):
            return func_expr.name                    # ex: "send", "broadcast"
...
def check(script: LuaScript, ctx: BuildContext) -> list[CheckError]:
    return Checker(ctx).check(script)
