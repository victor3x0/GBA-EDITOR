...
palettes à leur revisite, via `refresh()`.
...
Le correctif (chantier « L'écran resynchronisé à sa revisite ») : `refresh()`
re-lit le catalogue (`refresh_palette_catalog` du panneau), bon marché car il ne
relit que des banques déjà en mémoire, sans re-décoder l'image de l'asset.
`Window._show_screen` l'appelle au centre à chaque revisite ; les tests appellent
`refresh()` directement — un vrai `show()` réveille le canvas, qui ne survit pas
au mode headless (offscreen), alors que le câblage à vérifier est le refresh du
catalogue."""
