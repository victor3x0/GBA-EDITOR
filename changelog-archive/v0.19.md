...
- **Le sous-pixel vit dans la STRUCTURE, pas dans le langage.** `x/y/vx/vy` deviennent du Q8
  ...
  « que des entiers » (guide de scripting) n'est pas négociée ici : elle est la raison pour laquelle
  la ROM est rapide.
...
- **`math.lerp`/`math.ease` ne changent pas.** Un Q8 EST un entier ; ils interpolent l'un ou
  l'autre sans le savoir. Seul le guide de scripting gagne une phrase pour le dire. *(Tranché le
  2026-08-20.)*
...
`self.velocity`, `self:add_velocity`, le nouvel `self:apply_velocity`), et le guide de scripting.
