...
- **La liste des sons atteignables est EXHAUSTIVE, et ça tient au langage.** Le sous-ensemble
  Lua n'a pas de chaîne manipulable (référence de scripting) : une chaîne y est toujours un nom cité,
...
- **Les trois boîtes sont renommées.** `soundsbox` / `jinglesbox` / `musicsbox` deviennent
  ...
  `lua_compiler.py`, catalogue, référence de scripting, scripts de la démo. Chacune gagne
