...
- **Le document est écrit à la main, ses listes sont vérifiées.** La référence de scripting rejoint les
...
- **Les 67 `doc_anchor` d'`api_reference.json` ne mènent nulle part**, et le tooltip promet
  encore « doc (bientôt disponible) ». La référence de scripting documente le LANGAGE, pas chaque
...
- **La sémantique diffère de Lua, et c'est assumé.** `wait_until(cond)` ré-évalue son
  ...
  plutôt que de s'en priver, à charge pour la référence de scripting de porter l'écart en toutes lettres.
